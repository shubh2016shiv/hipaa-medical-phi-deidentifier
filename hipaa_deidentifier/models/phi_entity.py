...
def _find_full_atomic_entity(
    text: str, start_pos: int, end_pos: int, category: str
) -> Tuple[int, int]:
    """
    ...
    for match in matches:
        ...
        if (
            (match.start() <= entity_rel_start and match.end() >= entity_rel_end)
            or (match.start() >= entity_rel_start and match.start() < entity_rel_end)
            or (match.end() > entity_rel_start and match.end() <= entity_rel_end)
        ):
    ...
    if category == "URL":
        ...
        for url_pattern in url_patterns:
            ...
            for match in matches:
                if (
                    (
                        match.start() <= entity_rel_start
                        and match.end() >= entity_rel_end
                    )
                    or (
                        match.start() >= entity_rel_start
                        and match.start() < entity_rel_end
                    )
                    or (
...
def _expand_atomic_entities(entities: List[PHIEntity], text: str) -> List[PHIEntity]:
    ...
    for entity in entities:
        ...
        start, end = _find_full_atomic_entity(
            text, entity.start, entity.end, entity.category
        )
...
def _filter_whitelisted_headers(
    entities: List[PHIEntity], text: str
) -> List[PHIEntity]:
    """
...
def _resolve_overlaps(entities: List[PHIEntity]) -> List[PHIEntity]:
    ...
    sorted_entities = sorted(
        ...
        key=lambda e: (
            e.start,
            -(e.end - e.start),
            get_entity_priority(e.category),
            -e.confidence,
        ),
    )
    ...
    for current_entity in sorted_entities:
        ...
        for existing_entity in resolved:
            ...
            if (
                current_entity.start >= existing_entity.start
                and current_entity.end <= existing_entity.end
            ):
    ...
    for current_entity in resolved:
        ...
        if current_entity.category in ["URL", "PHOTO_ID"] and any(
            char.isdigit() for char in current_entity.text
        ):
            date_in_text = re.search(
                r"\d{4}[-/_]\d{1,2}[-/_]\d{1,2}", current_entity.text
            )
            if date_in_text:
        ...
        for final_entity in final_result:
            if max(current_entity.start, final_entity.start) < min(
                current_entity.end, final_entity.end
            ):
...
def merge_overlapping_entities(
    rule_entities: List[PHIEntity], ml_entities: List[PHIEntity], text: str
) -> List[PHIEntity]:
    """
