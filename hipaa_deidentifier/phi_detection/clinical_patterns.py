...
def detect_section_headers(text: str) -> List[PHIEntity]:
    ...
    for line_match in re.finditer(r"([^\n]+)", text):
        ...
        # MRN patterns
        mrn_match = re.search(
            r"(?:mrn|medical\s+record\s+number|record\s+number)\s*:\s*([^\n]+)",
            line,
            re.IGNORECASE,
        )
        if mrn_match:
        ...
        age_match = re.search(
            r"(?:age|patient\s+age)\s*:\s*(\d{1,3})", line, re.IGNORECASE
        )
        if age_match:
...
def detect_initials_and_nicknames(text: str) -> List[PHIEntity]:
    ...
    nickname_pattern = (
        r"\b[A-Z][a-z]+\s+\((?:nickname|aka|AKA|a\.k\.a\.|called)\s+[A-Z][a-z]+\)"
    )
...
def detect_facility_names(text: str) -> List[PHIEntity]:
    ...
    for line_match in re.finditer(r"([^\n]+)", text):
        ...
        for pattern in facility_patterns:
            facility_match = re.search(pattern, line)
            if facility_match:
...
def detect_relatives_and_contacts(text: str) -> List[PHIEntity]:
    ...
    pattern = (
        r"\b(" + "|".join(relationships) + r")\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b"
    )
...
def is_clinical_measurement(text: str) -> bool:
    ...
    all_patterns = (
        vital_patterns + lab_patterns + medication_patterns + medical_term_patterns
    )
...
def detect_long_numeric_ids(
    text: str, existing_entities: List[PHIEntity]
) -> List[PHIEntity]:
    """
