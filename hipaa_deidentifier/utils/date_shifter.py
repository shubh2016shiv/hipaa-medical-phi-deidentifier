    ...
    if fmt == "custom":
        ...
        if match:
            ...
            separators = [
                ...
                date_str[
                    len(match.group(1)) + len(match.group(2)) + 1 : len(match.group(1))
                    + len(match.group(2))
                    + 2
                ],
            ]
            ...
            if year_idx >= 0:
                ...
                result_parts = ["", "", ""]
                result_parts[year_idx] = (
                    f"{shifted.year if len(parts[year_idx]) == 4 else shifted.year % 100:0{len(parts[year_idx])}d}"
                )
                result_parts[(year_idx + 1) % 3] = (
                    f"{shifted.month:0{len(parts[(year_idx + 1) % 3])}d}"
                )
                result_parts[(year_idx + 2) % 3] = (
                    f"{shifted.day:0{len(parts[(year_idx + 2) % 3])}d}"
                )
