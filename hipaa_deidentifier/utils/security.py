...
def generate_secure_hash(text: str, salt: str, length: int = 12) -> str:
    ...
    if not salt or salt == "DEFAULT_SALT_REPLACE_IN_PRODUCTION":
        ...
        warnings.warn(
            "Using default salt for hashing. This is not secure for production use."
        )
...
class PseudonymManager:
    ...
    def get_pseudonym(
        self, entity_text: str, entity_type: str, patient_id: Optional[str] = None
    ) -> str:
        """
        ...
        format_template = (
            self.config.get("pseudonym_formats", {}).get(entity_type)
            or self.config.get("pseudonym_formats", {}).get("DEFAULT")
            or "{code}"
        )
