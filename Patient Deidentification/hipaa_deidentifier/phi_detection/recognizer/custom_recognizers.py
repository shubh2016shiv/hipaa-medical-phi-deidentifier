    ...
    def __init__(self, name="MRN", supported_entity="MRN", patterns=None):
        if patterns is None:
            patterns = [
                ...
                Pattern(
                    "mrn_labeled", 
                    r"\b(?:MRN|Med(?:ical)?\s*Record\s*(?:Number|No\.?)|Chart\s*(?:Number|No\.?))\s*[:#=\-]?\s*([A-Z0-9]{6,18})\b", 
                    0.95
                ),
                # MRN with hash/pound sign prefix - high confidence
                Pattern(
                    "mrn_hash_prefix",
                    r"\b(?:MRN|Med(?:ical)?\s*Record|Chart)?\s*#\s*([A-Z0-9]{6,18})\b",
                    0.9
                ),
                ...
                # State-based MRN formats - high confidence
                Pattern(
                    "mrn_state_format",
                    r"\bMRN[-:]?[A-Z]{1,2}[-:]?\d{1,2}[-:]?\d{1,5}\b",  # Format like MRN-AZ-44-22119
                    0.95
                ),
                # Name-based MRN formats - medium-high confidence
                Pattern(
                    "mrn_name_format",
                    r"\b[A-Z]{1,3}[-]?[A-Z]{1,5}[-]?\d{5,10}\b",  # Format like MR-SMITH-12345
                    0.85
                ),
                # Date-based MRN formats - medium-high confidence
                Pattern(
                    "mrn_date_format",
                    r"\bMRN[-:]?\d{4}[-:]\d{5,10}[-:]\d{1,2}\b",  # Format like MRN-2023-12345-01
                    0.85
                ),
                # Standalone MRN with context - medium confidence
                Pattern(
                    "mrn_with_context",
                    r"(?:patient|record|chart|account)(?:\s+(?:id|identification|number))?\s*[:#]?\s*([A-Z0-9]{6,18})\b",
                    0.65
                ),
                ...
                # MRN in context of DOB or other PHI - medium confidence
                Pattern(
                    "mrn_in_context",
                    r"\bDOB:.*?\bMRN:?\s*([A-Z0-9\-]{6,18})\b",  # Format like DOB: 01/01/1980 MRN: 12345678
                    0.75
                ),
            ]
