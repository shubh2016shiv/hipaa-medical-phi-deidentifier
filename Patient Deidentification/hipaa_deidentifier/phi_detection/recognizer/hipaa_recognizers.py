...
class FaxNumberRecognizer(PatternRecognizer):
    """
    Recognizes fax numbers with explicit fax labels.
    """
    def __init__(self, name="FAX_NUMBER", supported_entity="FAX_NUMBER", patterns=None):
        if patterns is None:
            patterns = [
                # Fax number with explicit label - captures only the phone number part
                Pattern(
                    "fax_labeled",
                    r"\b(?:Fax|FAX|Facsimile)\s*[:#=\-]?\s*(\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4})\b",
                    0.95
                ),
                # Fax number in parentheses format with label
                Pattern(
                    "fax_paren",
                    r"\b(?:Fax|FAX)\s*[:#=\-]?\s*(\(\d{3}\)\s*\d{3}[-.\s]?\d{4})\b",
                    0.95
                ),
                # Fax number with dash format and label
                Pattern(
                    "fax_dash",
                    r"\b(?:Fax|FAX)\s*[:#=\-]?\s*(\d{3}[-.\s]?\d{3}[-.\s]?\d{4})\b",
                    0.9
    ...
    def validate_result(self, pattern_text: str) -> bool:
        """
        Override to ensure we only match when there's a fax label.
        """
        # Check if the text contains fax-related keywords
        fax_keywords = ['fax', 'FAX', 'facsimile']
        return any(keyword in pattern_text for keyword in fax_keywords)
